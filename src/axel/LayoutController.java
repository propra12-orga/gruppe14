...
public class LayoutController {
	...
	//These are the original image sizes. Use them with a smaller field or a bigger window. Otherwise the field will be to big to display in the given JLayeredPane
	...
	private void initField()
	{
		...
		for(int i=0;i< this.heigth; i++)
			for(int j=0; j< this.width;j++)
			{
				...
				this.envField[i][j].setSize(LayoutController.ADAPT_PANEL_WIDTH, LayoutController.ADAPT_PANEL_HEIGTH);
	...
	{
		try {
			this.floorIMG =  ImageIO.read(new File(System.getProperty("user.dir") + "/src/graphics/floor.png"));
			this.floorIMG = ImageUtilities.resize(ADAPT_PANEL_WIDTH, ADAPT_PANEL_HEIGTH, this.floorIMG);
			this.solidWallIMG =  ImageIO.read(new File(System.getProperty("user.dir") + "/src/graphics/solidwall.jpg"));
			this.solidWallIMG = ImageUtilities.resize(ADAPT_PANEL_WIDTH, ADAPT_PANEL_HEIGTH, this.solidWallIMG);
			this.boxIMG =  ImageIO.read(new File(System.getProperty("user.dir") + "/src/graphics/box.png"));
			this.boxIMG = ImageUtilities.resize(ADAPT_PANEL_WIDTH, ADAPT_PANEL_HEIGTH, this.boxIMG);
			this.doorIMG =  ImageIO.read(new File(System.getProperty("user.dir") + "/src/graphics/door.png"));
			this.doorIMG = ImageUtilities.resize(ADAPT_PANEL_WIDTH, ADAPT_PANEL_HEIGTH, this.doorIMG);
			this.bombIMG = ImageIO.read(new File(System.getProperty("user.dir") + "/src/graphics/bomb.png"));
			this.bombIMG = ImageUtilities.resize(ADAPT_PANEL_WIDTH, ADAPT_PANEL_HEIGTH, this.bombIMG);
			this.explo1IMG =  ImageUtilities.applyWhiteTransparency(ImageIO.read(new File(System.getProperty("user.dir") + "/src/graphics/explosion1.png")));
			this.explo1IMG = ImageUtilities.resize(ADAPT_PANEL_WIDTH, ADAPT_PANEL_HEIGTH, this.explo1IMG);
			this.explo2IMG =  ImageUtilities.applyWhiteTransparency(ImageIO.read(new File(System.getProperty("user.dir") + "/src/graphics/explosion2.png")));
			this.explo2IMG = ImageUtilities.resize(ADAPT_PANEL_WIDTH, ADAPT_PANEL_HEIGTH, this.explo2IMG);
			this.explo3IMG = ImageUtilities.applyWhiteTransparency(ImageIO.read(new File(System.getProperty("user.dir") + "/src/graphics/explosion3.png")));
			this.explo3IMG = ImageUtilities.resize(ADAPT_PANEL_WIDTH, ADAPT_PANEL_HEIGTH, this.explo3IMG);
		} catch (IOException e) {
	...
	//Moves a player from startPos to endPos. Includes smoothed panel transition.
	...
	//Uses the TempOverlay JLabel to display a bomb that disappears after LIFETIME_BOMB
	...
	//Draws an explosion on the tile based on the orientation of the explosion. This depends on the location of the explosion center.
	//Orientation has to be distinguished between center, vertical and horizontal
